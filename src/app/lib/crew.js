// https://github.com/influenceth/sdk/blob/master/src/lib/crew.js

import Crewmate from "./crewmate.js";
import Station from "./station.js";

const CREWMATE_STACKING_BONUS_EFFICIENCY = [
  0.5, 1.0, 1.25, 1.375, 1.4375, 1.46875,
];
...
const getAbilityBonus = (
  ...
  if (notFurtherModified) {
    ...
    // Combine them all
    details.totalBonus =
      details.crewmatesMultiplier *
      details.stationMultiplier *
      details.foodMultiplier;
  }
...
const getAbilityBonusFromCrewmates = (abilityId, crewmates = []) => {
  ...
  crewmates.forEach((crewmate) => {
    ...
    const crewmateTraits =
      crewmate.traitIds || crewmate.Crewmate?.impactful || [];
    const crewmateCollection =
      crewmate.collectionId ||
      crewmate.Crewmate?.collection ||
    ...
    if (
      titleInfo.department &&
      ability.departments &&
      ability.departments[titleInfo.department]
    ) {
      const tier =
        titleInfo.tier +
        (crewmateCollection === Crewmate.COLLECTION_IDS.ARVAD_SPECIALIST
          ? 0.5
          : 0);
      const bonusPerMatch = ability.departments[titleInfo.department] * tier;
      const info = details.titles[crewmateTitle] || {
        matches: 0,
        bonusPerMatch,
        bonus: 0,
      };
    ...
    if (crewmateTraits.length > 0) {
      crewmateTraits.forEach((traitId) => {
        if (traitId && ability.traits && ability.traits[traitId]) {
          const info = details.traits[traitId] || {
            matches: 0,
            bonusPerMatch: ability.traits[traitId],
            bonus: 0,
          };
          info.matches++;
  ...
  if (details.class.classId) {
    details.class.multiplier =
      CREWMATE_STACKING_BONUS_EFFICIENCY[details.class.matches];
...
const getAbilityBonusFromStation = (station) => {
  ...
  if (station.stationType && station.population) {
    details.stationMultiplier = Station.getEfficiency(
      station.stationType,
      station.population
    );
  }
...
const getAbilityBonusFromFood = (timeSinceFed, crewmates = []) => {
  ...
  details.foodMultiplier = getFoodMultiplier(
    timeSinceFed,
    consumption,
    rationing
  );
